...
/**
 * 宽松请求（不检查 code，返回原始 JSON）
 * 用于清理操作：即使失败也不中断测试
 */
async function requestRaw(method, path, body = null, token = null) {
  const url = `${BASE_URL}${path}`;
  const headers = { 'Content-Type': 'application/json' };
  if (token) {
    headers.Authorization = `Bearer ${token}`;
...
async function runTests() {
  ...
  // 5a. 先查看是否已有授权记录（可能上次测试残留）
  const existingGrants = await get('/api/doctor/granted', patientToken);
  const oldAuth = existingGrants.data.find(a => a.doctor_id === doctorId);
  if (oldAuth) {
    // 撤销旧授权，确保本次是全新的 pending 记录
    await requestRaw('DELETE', `/api/doctor/revoke?auth_id=${oldAuth.id}`, null, patientToken);
    console.log(`  🔄 已撤销旧授权记录 (id=${oldAuth.id}, status=${oldAuth.status})`);
  }
